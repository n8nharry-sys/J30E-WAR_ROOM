...
export function TopSeller({ data }: { data: TopSellerRow[] }) {
  ...
  return (
    <div className="card h-full min-h-0 flex flex-col">
      ...
      <div className="grid grid-cols-[1.6rem_2.2rem_minmax(0,1fr)_7rem_7rem] gap-2.5 text-[10px] uppercase font-bold text-mut px-0.5 pb-1 shrink-0">
        <span />
        <span />
        <span>SMT</span>
      ...
      <div className="flex-1 flex flex-col justify-around">
        {rows.map((r) => (
          <div
            key={r.nik}
            className="grid grid-cols-[1.6rem_2.2rem_minmax(0,1fr)_7rem_7rem] gap-2.5 items-center py-1 border-t border-line first:border-0"
          >
            <div
